  ...
  sock.onmessage = (ev) => {
    ...
    switch (m.type) {
      ...
      case 'delta': {
        ...
        s.setSnap({
          tick: m.tick, owner, army: m.army, morale: m.morale ?? [], income: m.income ?? [], land: m.land, border: m.border ?? [], defScore: m.defScore, defWeak: m.defWeak, stance: m.stance, developing: m.developing,
